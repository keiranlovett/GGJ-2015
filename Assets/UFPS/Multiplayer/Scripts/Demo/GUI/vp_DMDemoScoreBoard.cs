...
public class vp_DMDemoScoreBoard : Photon.MonoBehaviour
{
	...
	public List<string> m_VisibleStatNames = new List<string>(new string[]	{	"Player", "Shots", "Frags", "Deaths", "Score"	});
			...
			if (!m_StatNamesChecked)
			{
				...
				if (m_VisibleStatNames.Count > 0)
				{
					if (vp_MPLocalPlayer.Instance != null)
					{
						...
						{
							if (!vp_MPLocalPlayer.Instance.Stats.Names.Contains(m_VisibleStatNames[v]))

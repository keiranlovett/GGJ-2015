...
public class vp_MPDemoChat : Photon.MonoBehaviour
{
	...
	protected virtual void OnEnable()
	{
		...
		vp_GlobalEvent<string, bool>.Register("ChatMessage", AddMessage);	// called from 'vp_MPDebug'
	...
	public static void AddMessage(string message, bool broadcast = true)
	{
		...
		if (m_Chat != null)
		{
			if (broadcast && (PhotonNetwork.connectionStateDetailed == PeerState.Joined))
				m_Chat.photonView.RPC("AddChatMessage", PhotonTargets.AllBuffered, message);
			else
				m_Chat.AddChatMessage(message, null);
		}

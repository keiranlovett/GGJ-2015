...
public class vp_MPRemotePlayer : vp_MPNetworkPlayer
{
	...
	protected int PlatformID
	{
		...
		set
		{
			...
			Transform platform = vp_MPConnection.GetTransformOfViewID(value);
			if ((platform != null)
	...
	protected virtual void UpdatePosition()
	{
		...
		if (PredictPosition && !Player.Dead.Active)	// do not attempt prediction on ragdolls
	...
	public override void FireWeapon(int weaponIndex, Vector3 position, Quaternion rotation, PhotonMessageInfo info)
	{
		...
		FireEvent fireEvent = new FireEvent(weaponIndex, position, rotation, Shots);	// TODO: pool?
	...
	public override void ReceivePlayerKill(PhotonMessageInfo info)
	{
		...
		if (NameTag != null)
			vp_Timer.In(1, () => { NameTag.Visible = false; });

...
public class vp_MPLocalPlayer : vp_MPNetworkPlayer
{
	...
	int PlatformID
	{
		get
		{
			return vp_MPConnection.GetViewIDOfTransform(FPPlayer.Platform.Get());
		}
	}
	...
	public override void OnPhotonSerializeView(PhotonStream stream, PhotonMessageInfo info)
    {
		...
		if (PlatformID == 0)
			...
			stream.SendNext((Vector3)Controller.m_PositionOnPlatform);	// position of player on current platform

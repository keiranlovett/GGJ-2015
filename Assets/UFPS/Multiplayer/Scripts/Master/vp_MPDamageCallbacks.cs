...
public class vp_MPDamageCallbacks : Photon.MonoBehaviour
{
	...
	protected virtual void TransmitDamage(Transform targetTransform, Transform sourceTransform, float damage)
	{
		...
		// 4) 'damage' is assumed to have already been deducted from the master damage
		//		handler so all we use it for here is make sure it's a positive number.
		//		overrides can do a lot more with it (see example in 'vp_DMDamageCallbacks')
		...
		int viewID = vp_MPConnection.GetViewIDOfTransform(targetTransform);
		if (viewID == 0)
			return;
		...
		// abort if target already died (no health to transmit)
		...
		photonView.RPC("ReceiveObjectHealth", PhotonTargets.Others, viewID, d.CurrentHealth);
	...
	/// a new health value. sent by the maste to keep client damagehandlers
	/// in sync
	/// </summary>
	[RPC]
